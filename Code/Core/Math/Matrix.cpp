...
{
    const float *m = reinterpret_cast<const float *>(&m_Column[0]);
    ...
    if (det == 0.0f)
    {
        // Matrix is not invertible, return identity or some sentinel value
    ...
    Matrix4 result;
    float *r = reinterpret_cast<float *>(&result.m_Column[0]);
